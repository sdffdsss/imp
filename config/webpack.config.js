...
const { NPM_CONFIG_FlAG, REACT_APP_NO_TOPO_IMG } = process.env;
...
// const hasDownloadTopoImg = REACT_APP_NO_TOPO_IMG !== 'true';
...
// const topoXmlDataPath = path.resolve('./public/static/topoXmlData.json');
// const UpdatePopup = require('version-update-webpack-plugin');
...
module.exports = function (webpackEnv) {
    ...
    //         'process.env.CURRENT_VERSION': `${Date.now()}.0.0`,
    //     });
    ...
    return {
        ...
        entry: isEnvDevelopment
            ...
            : // : {
              //       polyfill: ['core-js/es', 'react-app-polyfill/ie11', 'react-app-polyfill/stable'],
              //       main: paths.appIndexJs,
              //   },
              ['core-js/es', 'react-app-polyfill/ie11', 'react-app-polyfill/stable', paths.appIndexJs],
        output: {
            // The build folder.
        ...
        optimization: {
            ...
            splitChunks: {
                ...
                cacheGroups: {
                    ...
                    'oss-topo': {
                        name: 'chunk-oss-topo',
                        test: /[\\/]node_modules[\\/]oss-topo[\\/]/,
                        priority: 20,
                        chunks: 'all',
                        enforce: true,
                    },
                    'oss-alarm-window': {
                        name: 'oss-alarm-window',
                        test: /[\\/]node_modules[\\/]oss-alarm-window[\\/]/,
                        priority: 20,
                        chunks: 'all',
                        enforce: true,
                    },
        ...
        module: {
            ...
            rules: [
                ...
                //     include: paths.appSrc,
                // },
                ...(isEnvDevelopment ? [eslintLoaderConfig] : []),
                {
        ...
        plugins: [
            // hasDownloadTopoImg && new TopoImgWebpackPlugin(),
            ...
            new webpack.NormalModuleReplacementPlugin(/\/style\/index.less$/, (opt) => {
                if (!opt.rawRequest && /antd\/es\/(\w|-)*\/?style/.test(opt.context)) {
                    const request = opt.request.split('!');
                    if (request.length > 0) {
                        const last = request[request.length - 1];
                        if (/antd\/es\/(\w|-)+\/style/.test(last)) {
                            return;
                        }
                    }
                    request.pop();
                    const newPath = 'oss-ui/es/style/reset.less';
            ...
            // isEnvProduction && new UpdatePopup({ inject: false, envKey: 'CURRENT_VERSION' }),
            isEnvProduction && new OssUiThemeWebpackPlugin(themeConfig),

            new AntdDayjsWebpackPlugin({ plugins: ['localeData'] }),
        ].filter(Boolean),
