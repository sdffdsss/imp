...
const Index = forwardRef((props, ref) => {
  ...
  const disabledDate = (current, disabledDates) => {
    ...
    return (
      ...
      (current && current > moment().subtract("month", -3)) ||
      (current &&
        form.getFieldsValue().copyDate &&
        current <
          moment(form.getFieldsValue().copyDate[1]).subtract("days", -1)) ||
  ...
  const disableCopyDate = (current, disabledDates) => {
    return (
      (current &&
        current >
          moment().endOf("month").subtract("month", -1).endOf("month")) ||
      (current &&
        form.getFieldsValue().shcedulDate &&
        current > form.getFieldsValue().shcedulDate[0]) ||
      (disabledDates && disabledDates(current))
    );
  };
  ...
  const getData = async () => {
    ...
    const params = {
      ...
      updateEndDate:
        values.shcedulDate[1]?.format("YYYY-MM-DD") ||
        values.shcedulDate[0]?.format("YYYY-MM-DD"),
      copyBeginDate: values.copyDate[0]?.format("YYYY-MM-DD"),
      copyEndDate:
        values.copyDate[1]?.format("YYYY-MM-DD") ||
        values.copyDate[0]?.format("YYYY-MM-DD"),
      copyTypes: copyRole,
      provinceId,
      regionId,
      groupId,
    ...
    handleLoading(true);
    updateWorkingScheduleBatch(params).then((res) => {
      handleLoading(false);
      handleOk(res);
    });
  };
  ...
  const getCircle = (e) => {
    if (form.getFieldsValue().copyDate && e.copyDate) {
      ...
      const end =
        form.getFieldsValue().copyDate[1] || form.getFieldsValue().copyDate[0];
