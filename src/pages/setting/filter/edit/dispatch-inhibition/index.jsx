...
export default React.forwardRef((props, ref) => {
    ...
    React.useEffect(() => {
        if (!(props.modelType === 'new')) {
            ...
            if (ref.current && values && _.isArray(values)) {
                const data = values.find((item) => item.key === 'timeperiod')?.value;
                if (!_.isEmpty(data)) {
                        ...
                        return (
                            <Form.Item label="启用时间段" name="timeperiodSwitch" valuePropName="checked" wrapperCol={{ span: 2 }}>
                                <Field
                                    mode={mode}
                                    render={() => {
                                        return getFieldValue('timeperiodSwitch') ? '开' : '关';
                                    }}
                                    renderFormItem={() => {
                                        return (
                                            <Switch
                                                checked={getFieldValue('timeperiodSwitch')}
                                                checkedChildren="开"
                                                unCheckedChildren="关"
                                                size="small"
                                            ></Switch>
                            ...
                            return (
                                ...
                                >
                                    <Field
                                        ...
                                        renderFormItem={() => {
                                            return <TimePicker.RangePicker format="HH:mm" disabled={!getFieldValue('timeperiodSwitch')} />;
                                        }}
                                    />
                                </Form.Item>
                            );
