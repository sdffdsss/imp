...
const UploadComp = (props) => {
    ...
    const submitUpload = async () => {
        ...
        const { handleCancel,onUploadResult, type, majorId, provinceId, userName } = props;
    ...
    //         //window.open(url);
    //         //createFileFlow(fileData.name, url);
    //         const data = {
    //             source: uploadData.failureReasonFilePath,
    //         };
    //         const result = await api.fileDownload(data);
    //         blobDownLoad(result);
    //     } else {
    ...
    const downloadTemplate = async () => {
        ...
        if (props.type === exportType.bigCustomer) {
            result = await api.downloadBigCustomTemplate();
            blobDownLoad(result, `大客户平台业务故障导入模板.xlsx`);
        } else if (props.type === exportType.coreNetwork) {
            result = await api.downloadCoreNetworkTemplate();
            blobDownLoad(result, `核心网业务故障导入模板.xlsx`);
        }
    };
    ...
    return (
        <>
            <Modal title="导入" visible={props.isModalOpen} {...props} footer={<CustomModalFooter render={customModalFooterRender} />} onCancel={closeModal} width="600px">
