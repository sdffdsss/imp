    ...
    return treeNodes.map(({ children, title, ...props }) => {
        ...
        return (
            <TreeNode
                ...
                checkable={props.subjoin2 === 'person'}
                key={props.key}
                height={333}
            >
    ...
    return (
        ...
        >
            {({ direction, onItemSelect, selectedKeys }) => {
                if (direction === 'left') {
                    const checkedKeys = [...selectedKeys, ...selectUserList].map((e) => (e.userId ? e.userId : e));
                    return (
                        <Fragment>
                            {/* <Search
...
export default class Index extends React.PureComponent {
    constructor(props) {
        ...
        this.state = {
            visible: false,
            editRow: {},
            targetKeys: props.selectUserList.map((e) => (e.userId ? e.userId : e)),
            searchKey: '',
            expandedKeys: [],
    ...
    getUserGroupList = (data) => {
        ...
        if (data.ids && data.ids.length > 0) {
            data.ids = data.ids.map((item) => `${item}`);
        }
        ...
        return request('alarmResourceByeoms/queryDeptTree', {
            ...
            if (res && res.data) {
                const datas = res.data.filter((e) => e);
                getKeyDataList(datas);
                if (data.queryType === 'rootNode') {
                    ...
                    this.loopTreeData(treeData, newId, datas);
                    const transferData = [];
                    getDataList(treeData, transferData);
                    if (!this.state.searchKey) {
                        let treeId = getTreeIdByProvice(String(this.props.zoneId));
                        let treeIndex = datas.findIndex((item) => item.key === treeId);
                        let proviceObj = datas.filter((item) => item.key === treeId);
                    ...
                    const transferData = [];
                    getDataList(newData, transferData);
                    const expandedKeys = [];
                    if (transferData) {
                    ...
                    this.setState({
                        ...
                        treeData: this.disposeData(newData),
                    });
    ...
    saveData = () => {
        ...
        //     }
        // });
        if (this.props.editModleName === 'headId' && targetKeys.length > 1) {
            message.warn('监控中心负责人最多选择一个');
            return;
        }
        if (_isEmpty(numbers)) {
            flag = true;
        ...
        if (flag) {
            ...
            const callback = onChange(true, resultArr, this.props.editModleName);
            if (!callback) {
                this.restTreeData();
            }
        } else {
    ...
    onChange = (targetKeys) => {
        const { targetKeys: oldArr } = this.state;
        let newKeys = [...oldArr];
        if (targetKeys.length > oldArr.length) {
            // 新增值班人员
            targetKeys.forEach((e) => {
                if (!newKeys.includes(e)) {
                    newKeys.push(e);
                }
            });
        } else {
