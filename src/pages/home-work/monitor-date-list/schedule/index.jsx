...
const GroupSchedule = (props) => {
    ...
    useEffect(() => {
        if (scheduleTime && scheduleTime[1] && scheduleTime[0] && dataSource) {
            ...
            for (let i = 0; i <= 6; i += 1) {
                const a = i === 0 ? newTime.startOf('day').subtract('day') : newTime.startOf('day').subtract('day', -1);
                list.push({
                    title: (
                        <div>
                            <p>{`周${nzhcn.encodeS((a.day() || 7) === 7 ? '日' : a.day() || 7)}`}</p>
        ...
        if (!scheduleTime[1] && !scheduleTime[0]) {
            props.getAutoScheduleData({
                userGroup: dataSource,
                beginDate: scheduleTime[0],
                endDate: scheduleTime[1],
    ...
    return (
        <div>
            ...
            >
                <Row gutter={24}>
                    <Col span={12}>
                        <Form.Item labelCol={{ span: 6 }} label="归属省份" name="provinceId" required>
                            <Input disabled />
                        </Form.Item>
                    </Col>
                    <Col span={12}>
                        <Form.Item labelCol={{ span: 6 }} label="监控班组" name="groupId" required>
                            <Input disabled />
                        </Form.Item>
                    </Col>
                </Row>
                <Row gutter={24}>
                    <Col span={12}>
                        <Form.Item labelCol={{ span: 6 }} label="归属监控中心" name="centerId" required>
                            <Input disabled />
                        </Form.Item>
                    </Col>
                </Row>
                <Row>
                    <Col span={12}>
                        <Form.Item labelCol={{ span: 6 }} label="排班时间" name="scheduleTime" required>
                            <DateRangeTime
                ...
                <Row>
                    <Col span={24}>
                        <Form.Item labelCol={{ span: 3 }} label="排班人员" name="scheduleMan" required>
                ...
                <Row>
                    <Col span={24}>
                        <Form.Item labelCol={{ span: 3 }} label="排班示例">
                            <Table size="small" pagination={false} columns={exampleColumns} dataSource={exampleSource} />
                        </Form.Item>
                    </Col>
                </Row>
            </Form>
