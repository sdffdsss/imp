...
function WindowCard({
    ...
    const dispach = async (record, type, event) => {
        ...
        onRecord([!_isEmpty(alarmDetails) ? alarmDetails : record], type, true);
    };
    // useEffect(() => {
    //     const data = tabItem;
    //     // eslint-disable-next-line @typescript-eslint/no-unused-expressions
                    ...
                    return {
                        dom: (
                            <div className="" style={{ textAlign: 'center' }}>
                                ...
                                <span style={{ color: '#1890ff', cursor: 'pointer' }} type="link" onClick={(e) => dispach(record, 'addDispatch', e)}>
                                    追单
                                </span>{' '}
                                <span style={{ color: '#1890ff', cursor: 'pointer' }} type="link" onClick={(e) => dispach(record, 'clear', e)}>
                                    清除
                                </span>
                            </div>
                        ),
